...
export class Match {
  ...
  updateTeamNames() {
    if (this.countPlayers("a") > 0) {
      this.s.t.a.name = `${this.getPlayers("a")[0].name} Team`;
    }
    if (this.countPlayers("b") > 0) {
      this.s.t.b.name = `${this.getPlayers("b")[0].name} Team`;
    }
  }
  ...
  countPlayers(team) {
    return this.getPlayers(team).length;
  }
