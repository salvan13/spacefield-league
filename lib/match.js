...
export class Match {
    ...
    this.s = { // state
      m: { // match
        _: {
          ...
          ended: false,
          sleeping: false,
          time: cfg.isTraining ? Number.MAX_SAFE_INTEGER : MATCH_TIME,
          name: cfg.name,
          maxp: cfg.maxp,
          minp: cfg.minp
        }
      ...
      if (!this.s.m._.sleeping && !this.s.m._.ended) {
        if (this.s.m._.started) {
          ...
          if (!this.checkGoal()) {
            ...
            if (this.lastGoalTime - this.s.m._.time == 250) {
  ...
  updateBall(x, y, d, power) {
    ...
    if (!this.isFreePosition(this.s.ba._)) {
      const newPos = { py: this.s.ba._.py, px: this.s.ba._.px };
      if (L == d || R == d) {
        if (rand(2) == 1) {
          newPos.py++;
        } else {
  ...
  incrementStat(type, who, n = 1) {
    ...
    if (!stat[who]) {
      stat[who] = { name: this.s.p[this.clientIdToSocketId(who)].name, v: type == 'vote' ? 5 : 0, t: this.s.p[this.clientIdToSocketId(who)].team };
    }
    stat[who].v += n;
    const vote = {
      goal: 0.35,
  ...
  broadcast(event, data) {
    ...
    if (!event) {
      this.previousState = JSON.parse(JSON.stringify(this.s));
    }
  }
}
